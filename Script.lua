...
pcall(function()
    if game.CoreGui:FindFirstChild("MisterXHub") then game.CoreGui.MisterXHub:Destroy() end
    if L.PlayerGui:FindFirstChild("MisterXHub") then L.PlayerGui.MisterXHub:Destroy() end
end)

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "MisterXHub"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = (gethui and gethui()) or game.CoreGui or L:WaitForChild("PlayerGui")

local AutoBreak, AutoBreakIron, AutoCollect, AutoChopTrees = false, false, false, false
local AutoCollectWood, AutoCollectIron, AutoCollectMeat, AutoCollectEgg = false, false, false, false
local AutoChest, VoidMobs, NoclipActive = false, false, false
local StoneAura, TreeAura, KillAura, GodMode = false, false, false, false
local AuraRange = 25
local CustomSpeed = 16
local OpChests = {}
local MobNames = {"bear","snake","spider","shadow","treant","monster","archer","大触手","小触手","站桩章鱼怪","站桩野人弓箭手","章鱼怪","章鱼怪大副","章鱼怪海盗","chicken"}
...
local function tp(cf, offset)
    pcall(function()
        ...
        if root then
            ...
            root.CFrame = cf + (offset or Vector3.new(0, 1.5, 0))
        end
    end)
end
...
local function getTarget(mode)
    ...
    for _, obj in ipairs(W:GetDescendants()) do
        if obj:IsA("Model") or (mode:find("drop") and obj:IsA("BasePart")) then
            ...
            local m = false
            if mode == "stone_solid" and not isDrop and oN == "stone" and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "iron_solid" and not isDrop and (oN:find("iron") or oN:find("iron ore") or oN:find("iron stone")) and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "tree_solid" and not isDrop and (oN == "tree" or oN == "coconut tree") and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "stone_drop" and isDrop and oN:find("stone") and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "wood_drop" and isDrop and (oN:find("wood") or oN:find("log") or oN:find("branch") or oN:find("stick")) and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "iron_drop" and isDrop and oN:find("iron") and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "meat_drop" and isDrop and (oN:find("meat") or oN:find("food") or oN:find("steak")) and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "egg_drop" and isDrop and oN:find("egg") and not obj:IsDescendantOf(L.Character) then
                m = true
            elseif mode == "chest" and not isDrop and (oN:find("chest") or oN:find("crate") or oN:find("box")) and not (oN:find("tower") or pN:find("tower")) and not OpChests[obj] and not obj:IsDescendantOf(L.Character) then
                m = true
            end
            if m then
                ...
                if part and part.Position.Y > -50 then
                    ...
                    if dist < mD and (mode:find("solid") and dist > 1.5 or mode:find("drop") or mode == "chest") then
                        ...
                        nO = obj
                        nP = part
                        tP = obj:FindFirstChildWhichIsA("ProximityPrompt", true) or part:FindFirstChildWhichIsA("ProximityPrompt", true)
                    end
                end
            end
        end
    ...
    while true do
        ...
        if r then
            ...
            local function doLoot(m)
                ...
                if o and p and p.Parent then
                    act = true
                    tp(p.CFrame, Vector3.new(0, 0.5, 0))
                    local t = tick()
            ...
            elseif AutoChest then
                ...
                if o and p and p.Parent then
                    act = true
                    tp(p.CFrame, Vector3.new(0, 1.2, 0))
                    task.wait(0.08)
                    if pr then pcall(function() fireproximityprompt(pr) end) end
                    local cl = o:FindFirstChildWhichIsA("ClickDetector", true)
                    if cl then pcall(function() fireclickdetector(cl) end) end
        ...
        task.wait(act and 0.05 or 0.25)
    end
end)
        ...
        if StoneAura or TreeAura then
            ...
            if r then
                for _, obj in ipairs(W:GetDescendants()) do
                    ...
                    local isDrop = pN:find("drop") or oN:find("drop")
                    local match = false
                    if StoneAura and not isDrop and oN == "stone" and not obj:IsDescendantOf(L.Character) then
                        match = true
                    elseif TreeAura and not isDrop and (oN == "tree" or oN == "coconut tree") and not obj:IsDescendantOf(L.Character) then
                        match = true
                    end
                    if match then
                        local part = obj:IsA("BasePart") and obj or obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart", true)
                ...
                for _, obj in ipairs(W:GetChildren()) do
                    if obj:IsA("Model") and obj ~= L.Character then
                        ...
                        for _, bad in ipairs(MobNames) do
                            if name:find(bad:lower()) then isMob = true break end
                        end
                        if isMob and not name:find("mrbeast") then
                            local part = obj:IsA("BasePart") and obj or obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart", true)
                            if part and part.Position.Y > -50 and (part.Position - r.Position).Magnitude <= AuraRange then
                                if meleeHitRem then pcall(function() meleeHitRem:FireServer({obj}, {}) end) end
                            end
...
Tablet.Size = UDim2.new(0, 480, 0, 310)
Tablet.Position = UDim2.new(0.5, -240, 0.3, 0)
...
CloseBtn.Position = UDim2.new(0.92, 0, 0.16, 0)
...
MinBtn.Position = UDim2.new(0.84, 0, 0.16, 0)
...
ContentPanel.Size = UDim2.new(1, -165, 1, -48)
ContentPanel.Position = UDim2.new(0, 155, 0, 42)
...
local function addToggle(page, icon, text, defaultState, callback)
    ...
    txtL.Size = UDim2.new(1, -70, 1, 0)
    txtL.Position = UDim2.new(0, 38, 0, 0)
    ...
    statusL.Position = UDim2.new(1, -38, 0, 0)
...
FloatPill.Size = UDim2.new(0, 50, 0, 50)
...
CloseBtn.MouseButton1Click:Connect(function()
    AutoBreak, AutoBreakIron, AutoCollect, AutoChopTrees = false, false, false, false
    AutoCollectWood, AutoCollectIron, AutoCollectMeat, AutoCollectEgg = false, false, false, false
    AutoChest, VoidMobs, NoclipActive = false, false, false
    StoneAura, TreeAura, KillAura, GodMode = false, false, false, false
    CustomSpeed = 16
    if L.Character and L.Character:FindFirstChild("Humanoid") then
...
addButton(HomeTab, "⚡", "DELTA READY & STABLE", function() end)

addToggle(MainTab, "🛡️", "GOD MODE (INFINITE HEALTH/HUNGER)", false, function(s) GodMode = s end)
...
addButton(MainTab, "📏", "AURA RANGE (25 STUDS)", function(lbl)
    if AuraRange == 25 then AuraRange = 45
    elseif AuraRange == 45 then AuraRange = 70
    elseif AuraRange == 70 then AuraRange = 100
    else AuraRange = 25 end
...
addButton(PlayerTab, "🚀", "OPEN FLY GUI (V3)", function()
    pcall(function()
...
addToggle(CombatTab, "💀", "AUTO KILL AURA", false, function(s) KillAura = s end)
